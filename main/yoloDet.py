...
class YoloTRT():
    def __init__(self, library, engine, conf):
        ...
        self.categories =  ['bicycle','bus','car','carrier','cat','dog','motorcycle','movablesignage','person','scooter', 'stroller','truck','wheelchair','barricade','bench','bollard','chair','fire_hydrant','kiosk','parking_meter','pole','potted_plant','power_controller','stop','table','traffic_light','traffic_light_controller','traffic_sign','tree_trunk']

        # Assign a unique random color to each class
        self.colors = {category: [random.randint(0, 255) for _ in range(3)] for category in self.categories}
    ...
    def Inference(self, img):
        ...
        for j in range(len(result_boxes)):
            ...
            det["box"] = box
            if class_id <13 and class_id != 7:
                det["type"] = "movable"
            else:
                det["type"] = "fixed"

            det_res.append(det)
            #self.PlotBbox(box, img, color=self.colors[det["class"]], label="{}:{:.2f}".format(self.categories[int(result_classid[j])], result_scores[j]),)
